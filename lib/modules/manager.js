...
var readRC = exports.readRC = function () {
  try {
    return JSON5.parse(fs.readFileSync(skit_RC, 'UTF-8'));
  } catch (e) {
    // warn('读取 .skitrc 失败！');
  }
  return {};
...
var writeRC = exports.writeRC = function (rc) {
  fs.writeFileSync(SKIT_RC, (0, _stringify2.default)(rc, {}, 4), 'UTF-8');
};
exports.reloadRC = function () {
  var root = childProcess.execSync('npm root -g', {
    encoding: 'utf-8'
  }).split('\n')[0],
      modules = globby.sync(['@*' + sysPath.sep + 'skit-*', 'skit-*'], {
    cwd: root
  }).map(function (name) {
    var mt = name.match(skitModuleReg);
    if (mt && mt.length == 4) {
      return {
        name: mt[2],
        type: mt[3],
